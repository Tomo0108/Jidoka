...
import { useCallback, memo, useRef, ChangeEvent } from 'react';
...
const CustomNode = ({ id, data }: NodeProps<CustomNodeData>) => {
  const { shape, label, description, file } = data;
  const fileInputRef = useRef<HTMLInputElement>(null);
  const updateNodeData = useFlowStore(state => state.updateNodeData);

  const handleDataChange = useCallback((newData: Partial<Omit<CustomNodeData, 'id' | 'onChange'>>) => {
    updateNodeData(id, newData);
  }, [id, updateNodeData]);

  const onFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      handleDataChange({ file: event.target.files[0] });
    }
  };
