    ...
    <aside className="w-64 bg-background border-r border-border flex flex-col">
      ...
      <div className="flex-grow overflow-y-auto">
        <Accordion type="multiple" defaultValue={['item-1', 'item-2']} className="w-full">
          ...
          <AccordionItem value="item-2" className="border-accent/20">
            <AccordionTrigger className="px-4 hover:bg-accent/10 text-accent-foreground">
              <div className="flex items-center gap-2">
                <div className="p-1 bg-accent/20 rounded">
            ...
            <AccordionContent>
              <div className="p-4 bg-accent/5">
                ...
                >
                  {edgeTypes.map((edge) => (
                    <div key={edge.type} className="flex items-center space-x-3 p-2 rounded-md hover:bg-accent/10 transition-colors">
                      <RadioGroupItem 
                        value={edge.type} 
                        id={edge.type} 
                        className="border-accent data-[state=checked]:bg-accent data-[state=checked]:border-accent"
                      />
                      <Label htmlFor={edge.type} className="text-sm cursor-pointer text-accent-foreground font-medium flex-1">
                        {edge.label}
                      </Label>
                    </div>
                  ))}
