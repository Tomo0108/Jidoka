...
const CustomNode: React.FC<NodeProps<CustomNodeData>> = ({ data, selected }) => {
  ...
  return (
    <div className={cn(getShapeClasses(), selected && "ring-2 ring-accent ring-offset-2 ring-offset-background")}>
      <Handle 
        type="target" 
        position={Position.Top} 
        className="!bg-accent !border-accent-foreground !w-3 !h-3 !shadow-md opacity-0 hover:opacity-100 transition-opacity"
      />
      
      <div className={cn("px-3 py-2 text-center break-words font-medium", getLabelClasses())}>
        {label}
      </div>
