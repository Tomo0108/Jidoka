...
const shapeStyles = {
  rectangle: "w-52 h-40 rounded-md",
  diamond: "w-40 h-40 bg-transparent border-none", // Container is a transparent square
  parallelogram: "w-52 h-32 transform -skew-x-12",
  startEnd: "w-48 h-24 rounded-full",
  custom: "w-64 h-48 rounded-lg",
};

// Styles for the inner content wrapper
const contentWrapperBaseStyle = "p-3 flex flex-col h-full w-full";
...
const CustomNode = ({ id, data }: NodeProps<CustomNodeData>) => {
  ...
  return (
    <TooltipProvider>
      <div className={cn(containerBaseStyle, shapeStyles[shape])}>
        {/* Special background for non-rectangular shapes */}
        ...
        <div className={cn(contentWrapperBaseStyle, contentShapeStyles[shape])}>
          ...
          <div className="relative flex-grow flex flex-col min-h-0">
            <Textarea
              placeholder="詳細..."
              className="nodrag font-chat text-sm flex-grow w-full h-full resize-none pr-8"
              value={description || ''}
