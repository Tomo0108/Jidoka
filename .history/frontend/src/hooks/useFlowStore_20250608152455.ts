...
import { temporal as temporalMiddleware, type TemporalState } from 'zustand/middleware';
...
// Zustand's temporal middleware has a type issue with create(), so we cast it.
const temporal = temporalMiddleware as <T>(fn: (set: (fn: (state: T) => T) => void, get: () => T) => T) => (set: (fn: (state: T) => T) => void, get: () => T) => T;
...
export const useTemporalStore = <T, >(
  ...
  // @ts-ignore
  return useFlowStore(state => selector(state.temporal), equality);
}
